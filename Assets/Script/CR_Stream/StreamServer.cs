...
namespace GameViewStream
{
    ...
    public sealed class StreamServer : MonoBehaviour
    {
        ...
        [SerializeField] private float clientTimeout   = 5f;
        [Tooltip("Maximum JPEG frames held in the shared queue waiting for decode workers. "
            ...
            while (!_cts.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (!NetworkProtocol.TryParseHeader(
                            ...
                            out ushort _,       // clientId in packet ignored — server maps by endpoint
                            out uint   frameId,
                            out int    payloadSize))
                    {
